...
import (
	...
	samplev1alpha1 "github.com/baichinger/sample-operator/pkg/apis/sample/v1alpha1"
...
func (r *ReconcileSample) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	reqLogger.Info("Instance", "object", instance)

	key := types.NamespacedName{Namespace: request.Namespace, Name: "foo"}
	secret := &corev1.Secret{}
	err = r.client.Get(context.TODO(), key, secret)
	reqLogger.Info("Secret", "object", secret)
	...
	reqLogger.Info("Skip reconcile: Pod already exists", "Pod", found)
	return reconcile.Result{}, nil
}
